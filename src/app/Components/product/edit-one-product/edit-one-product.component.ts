...
export class EditOneProductComponent implements OnInit
{
    ...
    reset()
    {
        // remove the new shop specific
        this.initialProduct.shopSpecifics = this.initialProduct.shopSpecifics.filter(ss => ss.id != -1);
        this.product = Operation.deepCopy(this.initialProduct);
        this.initDummyStruct();
    ...
    {
        // the shop already used are forbidden
        this.newShopStruct.forbiddenShops = this.product.shopSpecifics.map(ss => ss.shop);
    ...
    createNewShopSpecific(selectedShop: IdNameDto): void
    {
        ...
        // add to initialProduct means need to remove it in reset
